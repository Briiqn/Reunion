...
@PacketInfo(side = PacketSide.CONSOLE_C2S, id = 151, supports = {39, 78})
public final class ConsoleTradeItemC2SPacket extends ConsoleC2SPacket {

  private int containerId, offer;

  public ConsoleTradeItemC2SPacket() {
  }
  ...
  public void handle(ConsoleSession session) {
    session.setTradeWindowId((byte) containerId);
    ByteBuf payload = Unpooled.buffer(4);
    payload.writeInt(offer);
    PacketManager.sendToJava(session.getJavaSession(),
        new JavaCustomPayloadC2SPacket("MC|TrSel", payload));
  }
}
