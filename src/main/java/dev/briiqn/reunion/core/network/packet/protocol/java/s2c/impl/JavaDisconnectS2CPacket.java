...
@PacketInfo(side = PacketSide.JAVA_S2C, id = 0x40, supports = {47})
@Log4j2
public final class JavaDisconnectS2CPacket extends JavaS2CPacket {

  private String reason;

  public JavaDisconnectS2CPacket() {
  }

  @Override
  public void read(ByteBuf buf) {
    reason = readJavaString(buf);
  }

  @Override
  public void handle(JavaSession session) {
    String parsedReason = "Disconnected";
    try {
      parsedReason = parseJsonText(JSON.parseObject(reason));
    } catch (Exception e) {
      parsedReason = reason;
    }
    log.debug("[disconnect] " + parsedReason);
    session.setDisconnectReason(parsedReason);
    if (session.getJavaChannel() != null) {
      session.getJavaChannel().close();
    }
  }
