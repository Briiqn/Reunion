...
import dev.briiqn.reunion.core.network.packet.protocol.console.s2c.impl.ConsoleSetHealthS2CPacket;
...
@PacketInfo(side = PacketSide.JAVA_S2C, id = 0x06, supports = {47})
public final class JavaUpdateHealthS2CPacket extends JavaS2CPacket {

  private float health;
  private int food;
  private float saturation;

  public JavaUpdateHealthS2CPacket() {
  }

  @Override
  public void read(ByteBuf buf) {
    health = buf.readFloat();
    food = VarIntUtil.read(buf);
  ...
  public void handle(JavaSession session) {
    ...
    cs.setHealth(health);
    cs.setFoodLevel(food);

    PacketManager.sendToConsole(cs,
        new ConsoleSetHealthS2CPacket(health, (short) food, saturation));
  }
}
